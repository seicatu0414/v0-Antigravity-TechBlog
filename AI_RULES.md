...
- **ドキュメント言語**: `implementation_plan.md` などの計画書やドキュメントは、原則として日本語で記述すること。
...
## 7. Git運用 (Git Workflow)
- **機能開発**: 新機能を追加する際は、必ず `Feature_XXX` (XXXはチケット名や適切な機能名) のブランチを作成し、そのブランチで作業・コミットを行うこと。
- **コミットメッセージ**: 変更内容が明確に分かるように記述すること。


## 8. 開発プロセス (Development Process)
- **ローカルファースト**: まずは完全にローカル環境で動作するように開発を行うこと。
- **外部サービス**: AWSなどの外部クラウドサービスへの接続は、ユーザーからの明示的な指示がない限り避けること。ローカルでのモックや代替手段を使用すること。
- **言語**: ユーザーとの対話、計画書、コミットメッセージ、PR、コード内のコメントなど、全てのコミュニケーションとドキュメントは**日本語**で行うこと。
